...
"multiple.kernel" = function( x ) {
    ...
    D_Kernels = list()
    D_Kernels = unlist(lapply(1:length(allk),FUN=function(l,Diff_sort_fun=Diff_sort,allk_fun=allk,Diff_fun=Diff,sigma_fun=sigma,KK_fun=KK) {
        if(allk_fun[l]<(nrow(x)-1)) {
    ...
    D_Kernels[1:length(D_Kernels_first)] = D_Kernels_first
    rm(D_Kernels_first)
