function [y, S, F, ydata,alphaK,timeOurs,converge] = GB_OURS_fast(X, c,no_dim, k, ifimpute,normalize,ytrue)
...
for iter = 1:NITER
    ...
    for i = 1:size(D_Kernels,3)
        DD(i) = mean(sum(Dall(:,:,i)-diag(diag(Dall(:,:,i)))));
    end
    alphaK0 = umkl_bo(DD);
...
for l = 1:length(allk)
    ...
    for j = 1:length(sigma)
        ...
        %Kernels(:,:,KK+t) = (W + W')/2;
        Kernels(:,:,KK+t) = normpdf(Diff,0,sigma(j)*Sig);
        t = t+1;
    end
end
...
for i = 1:size(Kernels,3)
    ...
    D_Kernels(:,:,i) = (diag(G)*ones(1,length(G)) +ones(length(G),1)*diag(G)' - 2*G)/2;
